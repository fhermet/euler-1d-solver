...
$$\hat{\mathbf{F}}_{i+1/2} = \begin{pmatrix} \rho^\ast u^\ast \\ \rho^\ast (u^\ast)^2 + p^\ast \\ u^\ast (E^\ast + p^\ast) \end{pmatrix}$$

où $(\rho^\ast, u^\ast, p^\ast)$ sont les variables primitives de la solution du problème de Riemann en $\xi = 0$, et $E^\ast = p^\ast / (\gamma - 1) + \tfrac{1}{2} \rho^\ast (u^\ast)^2$ l'énergie totale correspondante.

La détermination de $(\rho^\ast, u^\ast, p^\ast)$ passe par :
1. Le calcul de la pression intermédiaire $p^\ast$ par itération de Newton (cf. §2.4).
2. L'identification du type de chaque onde (choc ou détente) à partir du signe de $p^\ast - p_L$ et $p^\ast - p_R$.
...
- **Coût élevé** : chaque interface nécessite une itération de Newton (typiquement 3 à 6 itérations) pour trouver $p^\ast$, plus l'échantillonnage de la solution. Cela représente un coût bien supérieur aux solveurs approchés.
...
$$\mathbf{U}_L \xrightarrow{S_L} \mathbf{U}^\ast_{\text{HLL}} \xrightarrow{S_R} \mathbf{U}_R$$

L'état intermédiaire $\mathbf{U}^\ast_{\text{HLL}}$ est déterminé par la conservation intégrale : en intégrant les équations de conservation sur le domaine de dépendance borné par $S_L$ et $S_R$, on obtient :

$$\mathbf{U}^\ast_{\text{HLL}} = \frac{S_R \mathbf{U}_R - S_L \mathbf{U}_L + \mathbf{F}_L - \mathbf{F}_R}{S_R - S_L}$$
...
Le schéma HLLC (*HLL-Contact*) corrige ce défaut en ajoutant une **troisième onde** $S^\ast$ correspondant à l'onde de contact. Le problème de Riemann approché comporte alors trois ondes et quatre états, comme le problème exact.

### Principe : quatre régions

Le modèle HLLC divise le plan $(x, t)$ en quatre régions séparées par trois ondes :

$$\mathbf{U}_L \xrightarrow{S_L} \mathbf{U}^\ast_L \xrightarrow{S^\ast} \mathbf{U}^\ast_R \xrightarrow{S_R} \mathbf{U}_R$$
...
- **Région étoile gauche** ($S_L < \xi < S^\ast$) : état intermédiaire $\mathbf{U}^\ast_L$.
- **Région étoile droite** ($S^\ast < \xi < S_R$) : état intermédiaire $\mathbf{U}^\ast_R$.
- **Région droite** ($\xi > S_R$) : état non perturbé $\mathbf{U}_R$.

Les deux états étoile partagent la **même pression** $p^\ast$ et la **même vitesse** $u^\ast = S^\ast$ (conditions de Rankine-Hugoniot à travers l'onde de contact), mais ont des **densités différentes** $\rho^\ast_L \neq \rho^\ast_R$. C'est précisément cette différence de densité que HLL ne pouvait pas capturer.
...
### Étape 2 : Vitesse de contact $S^\ast$

La vitesse de l'onde de contact $S^\ast$ est déterminée par la conservation de la quantité de mouvement à travers les ondes. En écrivant les relations de Rankine-Hugoniot pour les deux ondes extérieures (entre $\mathbf{U}_L$ et $\mathbf{U}^\ast_L$ d'une part, $\mathbf{U}^\ast_R$ et $\mathbf{U}_R$ d'autre part), et en imposant l'égalité des pressions étoile ($p^\ast_L = p^\ast_R$), on obtient :

$$S^\ast = \frac{p_R - p_L + \rho_L u_L (S_L - u_L) - \rho_R u_R (S_R - u_R)}{\rho_L (S_L - u_L) - \rho_R (S_R - u_R)}$$
...
- **Résultat** : $S^\ast$ est la vitesse commune des états étoile. Elle correspond à la vitesse du fluide dans la zone intermédiaire, c'est-à-dire la vitesse de la discontinuité de contact.

### Étape 3 : États intermédiaires $\mathbf{U}^\ast_K$
...
$$\mathbf{U}^\ast_K = \rho_K \frac{S_K - u_K}{S_K - S^\ast} \begin{pmatrix} 1 \\ S^\ast \\ E_K / \rho_K + (S^\ast - u_K)\bigl[S^\ast + p_K / (\rho_K (S_K - u_K))\bigr] \end{pmatrix}$$

**Interprétation terme par terme** :

- **Facteur $\rho_K (S_K - u_K) / (S_K - S^\ast)$** : ce ratio traduit la compression ou la dilatation du fluide entre l'état $K$ et l'état étoile. Il est déterminé par la conservation de la masse à travers l'onde $S_K$.

- **Composante 1** (masse) : la densité étoile est $\rho^\ast_K = \rho_K (S_K - u_K) / (S_K - S^\ast)$.

- **Composante 2** (quantité de mouvement) : la vitesse dans la zone étoile est $S^\ast$, donc $(\rho u)^\ast_K = \rho^\ast_K \cdot S^\ast$.
...
$$\hat{\mathbf{F}}_{i+1/2} = \begin{cases} \mathbf{F}_L & \text{si } S_L \geq 0 \\[4pt] \mathbf{F}^\ast_L = \mathbf{F}_L + S_L (\mathbf{U}^\ast_L - \mathbf{U}_L) & \text{si } S_L < 0 \leq S^\ast \\[4pt] \mathbf{F}^\ast_R = \mathbf{F}_R + S_R (\mathbf{U}^\ast_R - \mathbf{U}_R) & \text{si } S^\ast < 0 < S_R \\[4pt] \mathbf{F}_R & \text{si } S_R \leq 0 \end{cases}$$
...
2. **$S_L < 0 \leq S^\ast$** : l'onde la plus rapide à gauche a déjà traversé l'interface, mais l'onde de contact est encore à droite. L'interface est dans la région étoile gauche. Le flux $\mathbf{F}^\ast_L$ est obtenu par la relation de Rankine-Hugoniot : $\mathbf{F}^\ast_L = \mathbf{F}_L + S_L (\mathbf{U}^\ast_L - \mathbf{U}_L)$.

3. **$S^\ast < 0 < S_R$** : l'onde de contact a traversé l'interface, mais l'onde droite ne l'a pas encore atteinte. L'interface est dans la région étoile droite. Le flux $\mathbf{F}^\ast_R$ est obtenu de manière symétrique.
